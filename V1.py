...
if __name__ == '__main__':
    ...
    for primer in New_FPrimer:
        ...
        output_file.close() #JAN26, should free up PrimerComboFasta
       ...
       for seq_record in SeqIO.parse(inFile,'fasta'):
            ...
            #try:
                ...
                for targetsite in crRNA2:
                    ...
                    crRNA_count = crRNA_count + 1
                    os.getcwd()            
       inFile.close()#Jan26, should fix Infile access issues
...
if __name__ == '__main__':
    #Lets attempt to get output_sets for just the best primer. 
    if str(config.get('Features', "Run_Cas-off")) == 'Yes':
        print("[Primed Sherlock] Prepping data for Cas-Offinder")
        PrimedOutputLoc = config.get('main','Primed_RPA_Output')
        os.chdir(PrimedOutputLoc)
            ...
            for amplicons in Amp:
                if amplicons <= int(theta) and amplicons >= int(beta) :
                   if MaxBackground[time_count] <=  int(maxback):
                    ...
                    #print(NewRPrimerSite)
                time_count = time_count + 1   
    #Okay so now we generated the datasets lets go through them and only isolate the best run.
    Best_New_Amp = []
    Best_New_FPrimer = []
    Best_New_RPrimer = []
    Best_New_MaxBackground = []
    Best_NewFPrimerSite = []
    Best_NewRPrimerSite = []
    Best_BindingSiteNew = []
    Best_BindingSiteLoc = []
    ...
    FPRIMER = primers[0]
    RPRIMER = primers[1]
    primer2 = second_best_primer.split("_")
    FPRIMER2 = primer2[0]
    RPRIMER2 = primer2[1]
    new_time_count = 0
    for amplicons in New_Amp:
        if FPRIMER == str(New_FPrimer[new_time_count]):
            if RPRIMER == str(New_RPrimer[new_time_count]):
                Best_BindingSiteNew.append(BindingSiteNew[new_time_count])
                Best_BindingSiteLoc.append(BindingSiteLoc[new_time_count])
                Best_New_Amp.append(New_Amp[new_time_count])
                Best_New_FPrimer.append(New_FPrimer[new_time_count])
                Best_New_RPrimer.append(New_RPrimer[new_time_count])
                Best_New_MaxBackground.append(New_MaxBackground[new_time_count])
                Best_NewFPrimerSite.append(NewFPrimerSite[new_time_count])
                Best_NewRPrimerSite.append(NewRPrimerSite[new_time_count])
        if FPRIMER2 == str(New_FPrimer[new_time_count]):
            if RPRIMER2 == str(New_RPrimer[new_time_count]):
                Best_BindingSiteNew.append(BindingSiteNew[new_time_count])
                Best_BindingSiteLoc.append(BindingSiteLoc[new_time_count])
                Best_New_Amp.append(New_Amp[new_time_count])
                Best_New_FPrimer.append(New_FPrimer[new_time_count])
                ...
                Best_NewFPrimerSite.append(NewFPrimerSite[new_time_count])
                Best_NewRPrimerSite.append(NewRPrimerSite[new_time_count])
        new_time_count = new_time_count + 1
        #Lets place our first and second best primers as the input info as if its an output_sets.csv input.
    New_Amp = Best_New_Amp
    New_FPrimer = Best_New_FPrimer
    New_RPrimer = Best_New_RPrimer
    New_MaxBackground = Best_New_MaxBackground
    NewFPrimerSite = Best_NewFPrimerSite
    NewRPrimerSite = Best_NewRPrimerSite
    BindingSiteNew = Best_BindingSiteNew
    BindingSiteLoc = Best_BindingSiteLoc    
    #Now lets figure out the amplicon pairs
    try:
        Input_File = config.get('main','Input.fna_Location')
    except:
        print("[Primed Sherlock] Unable to locate Input.fna file")
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("[Primed Sherlock] Remade Primer Combo Fasta Directory")
if __name__ == '__main__':
    ...
    #Grabs Primer Sites
        ...
        ############################################################################## 
        #It compares the files in the On target folder with the values to see if non compatable strains exist".    
        os.chdir(working_dir)
    ...
    if Wintercontingency == "Yes":
        ...
        #For Second Best Primer & crRNA Pair
        A2MasterMatch = []
        A2MasterContingency = []
        A2MasterContingencyOff = []
        ContingencyMatch = []
        ContingencycrRNA = []
        ContingencycrRNA2 = []
        UniqueContingencyMatch = []
        ...
        UniqueContingencycrRNA2 = []
        for file in Contingency_files:
            if file.startswith(str(second_best_primer)): 
                crRNALIST = list(crRNA2.split(","))
                if file.endswith(str(crRNALIST[0]) + ".csv"):
                    Contingency_Data = pd.read_csv(file)
                    columns = list(Contingency_Data.columns.values.tolist()) 
                    ContingencyMatch = Contingency_Data[Contingency_Data.columns[0]]
                    ...
                    for unique in ContingencycrRNA2:
                        UniqueContingencycrRNA2.append(unique)
                        UniqueContingencycrRNA2 = list(dict.fromkeys(UniqueContingencycrRNA2))
                    A2MasterMatch.append(UniqueContingencyMatch)
                    A2MasterContingency.append(UniqueContingencycrRNA)
                    A2MasterContingencyOff.append(UniqueContingencycrRNA2)
            if file.startswith(str(second_best_primer)): 
                crRNALIST = list(crRNA2.split(","))
                if file.endswith(str(crRNALIST[1]) + ".csv"):
                    Contingency_Data = pd.read_csv(file)
        ...
        #List of 1st best
        list_1b_seq = []
        MasterContingency = MasterContingency + MasterContingencyOff
        for value in MasterContingency:
            for value2 in value:
                list_1b_seq.append(value2)
        A2MasterContingency = A2MasterContingency + A2MasterContingencyOff
        #List of 2nd best 
        list_2b_seq = []
        for value in A2MasterContingency:
            for value2 in value:
                list_2b_seq.append(value2)
        os.chdir(working_dir)
        #Now we have a list of sequences to make an alignment of the primers + amp of.
        
        #This next part just grabs the needed files. No need to generate thousands
        primers = current_best_primer.split("_")
        FPRIMER = primers[0]
        RPRIMER = primers[1]
        primer2 = second_best_primer.split("_")
        FPRIMER2 = primer2[0]
        RPRIMER2 = primer2[1]
        #Reforked original code for winter contingency with edit of searching for best & second best mismatch
        from Bio import SeqIO
        n = 0
        sequence = []
        seq_files = []
        ...
        for primer in New_FPrimer:
            ...
            n = n + 1
if __name__ == '__main__':
    #Lets delete everything but the PrimerComboFasta
    try:
        input_file.close()
    except:
        catch = 1
    ...
    dir_list = [ "ContingencycrRNA_Pairs", "PrimerVer", "PrimerCheck", "Cas-OffInput", "crRNA_Pairs", "TempAlignDir", "TempDIR", 'On', 'Off', 'crRNA_Pairs', 'Cas-On-Temp', 'Cas-On-Out', 'Cas-ONInput', 'Cas-On']
    os.chdir(working_dir)
    directories = [f for f in os.listdir('.') if os.path.isdir(f)]
    directoryfiles = [f for f in os.listdir('.') if os.path.isfile(f)]
    for d in directories:
...
#%%
    ...
    import shutil as sh
    path = os.getcwd()
    os.chdir(working_dir + "\crRNA_Pairs") 
    path = os.getcwd()
    crRNAfiles = os.listdir(path)
    try:
        os.chdir(Input_File)
...
if __name__ == '__main__':
    ...
    for values in Unique_primer:
        ...
        if Current_Unique_combined_all < best_all:
            ...
            if Current_Unique_combined_all >= second_best:
                if Current_Unique_primer != current_best_primer:
                    second_best = Current_Unique_combined_all
                    Second_Best_Amp = Current_Unique_amp #Current_Best_Amp
                    crRNA2 = Current_Unique_crRNApair
                    second_best_primer = Current_Unique_primer
    ...
    try:
            with open("Final_Output.csv", "a") as output_file:
                output_file.write("\n")
                output_file.write("\n")
                output_file.write("PrimedSherlock Contingency Results" + "," + "\n")
                output_file.write("Best RPA Primer Set" + "," + current_best_primer + "," + "\n" + "Amplicon Size" + "," + str(Current_Best_Amp) + "," + "\n" + "Total Matches within 4 bases" + "," + str(best_all) + "," + "\n")
                output_file.write("Best crRNA" + "," + crRNA1 + "," + "\n")
                output_file.write("Input strains not covered" + "," + "\n")
                count = 0
